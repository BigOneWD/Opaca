...
reconciliation, policy, and AUTO authority all pass.
"""
...
from tests.helpers import DEFAULT_PRICES, make_order, make_proposal
...
def test_live_paper_mutation_buy_one_sgov(tmp_path: Path) -> None:
    ...
    warning = (
        ...
        f"to {PAPER_ENDPOINT}. Live money is still forbidden."
    )
    ...
    store = SQLiteStore(tmp_path / "live-paper.sqlite")
    ...
    proposal = make_proposal(
        "live-buy-1-sgov",
        [make_order("live-buy-1-sgov", 0, "SGOV", Side.BUY, "1", DEFAULT_PRICES["SGOV"])],
    )
